...
class CompanySelector(QDialog):
    def __init__(self) -> None:
        ...
        refresh_btn = QPushButton("譖ｴ譁ｰ")
        select_btn = QPushButton("驕ｸ謚・)
    ...
    def load_companies(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "蜿門ｾ怜､ｱ謨・, f"莨夂､ｾ荳隕ｧ縺ｮ蜿門ｾ励↓螟ｱ謨励＠縺ｾ縺励◆: {e}")
...
class SettingsPage(QWidget):
    def __init__(self, company: str) -> None:
        ...
        layout = QVBoxLayout()
        # 蜍伜ｮ夂ｧ醍岼險ｭ螳夲ｼ郁｣懷勧遘醍岼繝ｻ鞫倩ｦ∝呵｣懶ｼ・        acct_group = QGroupBox("蜍伜ｮ夂ｧ醍岼險ｭ螳夲ｼ郁｣懷勧遘醍岼繝ｻ鞫倩ｦ∝呵｣懶ｼ・)
        ag = QFormLayout()
        self.acct_name_edit = QLineEdit()
        self.acct_subs_edit = QLineEdit()
        self.acct_summaries_edit = QLineEdit()
        ag.addRow("蜍伜ｮ夂ｧ醍岼蜷・, self.acct_name_edit)
        ag.addRow("陬懷勧遘醍岼・医き繝ｳ繝槫玄蛻・ｊ・・, self.acct_subs_edit)
        ag.addRow("鞫倩ｦ∝呵｣懶ｼ医き繝ｳ繝槫玄蛻・ｊ・・, self.acct_summaries_edit)
        abtns = QHBoxLayout()
        acct_save = QPushButton("菫晏ｭ・)
        acct_load = QPushButton("隱ｭ霎ｼ")
        ...
        layout.addWidget(QLabel("險ｭ螳・))
        # Output default config
        out_group = QGroupBox("蜃ｺ蜉帛・縺ｮ譌｢螳夊ｨｭ螳・)
        og = QVBoxLayout()
        self.default_hint = QLabel("蜃ｺ蜉帙・繝ｼ繧ｸ縺ｫ蛻晄悄陦ｨ遉ｺ縺輔ｌ繧区里螳壹・蜃ｺ蜉帛・繧剃ｼ夂､ｾ縺斐→縺ｫ險ｭ螳壹〒縺阪∪縺吶ょ盾辣ｧ縺ｯ蜃ｺ蜉帙・繝ｼ繧ｸ縺ｧ陦後＞縺ｾ縺吶・)
        ...
        # Archive base dir
        self.archive_label = QLabel("繝ｩ繝吶Ν莉倥￠隍・｣ｽ縺ｮ菫晏ｭ倥・繝ｼ繧ｹ繝輔か繝ｫ繝・井ｼ夂､ｾ蛻･・・)
        self.archive_base_edit = QLineEdit()
        form.addRow(self.archive_label, self.archive_base_edit)
        og.addLayout(form)
        save_btn = QPushButton("菫晏ｭ・)
        ...
        llm_group = QGroupBox("LLM險ｭ螳夲ｼ井ｼ夂､ｾ蛻･繧ｪ繝ｼ繝舌・繝ｩ繧､繝会ｼ・)
        lg = QVBoxLayout()
        ...
        llm_form.addRow("Override菴ｿ逕ｨ(1/0)", self.llm_use_override)
        ...
        llm_save = QPushButton("LLM險ｭ螳壹ｒ菫晏ｭ・)
        ...
        # LLM logs quick view
        logs_group = QGroupBox("LLM繝ｭ繧ｰ・域怙霑托ｼ・)
        lgl = QVBoxLayout()
        self.llm_logs = QListWidget()
        refresh = QPushButton("繝ｭ繧ｰ譖ｴ譁ｰ")
    ...
    def on_save_account_setting(self) -> None:
        ...
        if not name:
            QMessageBox.warning(self, "蜈･蜉帙お繝ｩ繝ｼ", "蜍伜ｮ夂ｧ醍岼蜷阪ｒ蜈･蜉帙＠縺ｦ縺上□縺輔＞")
            return
        try:
            r = requests.post(
            ...
            if r.ok:
                QMessageBox.information(self, "菫晏ｭ・, "蜍伜ｮ夂ｧ醍岼險ｭ螳壹ｒ菫晏ｭ倥＠縺ｾ縺励◆")
            else:
                QMessageBox.warning(self, "菫晏ｭ伜､ｱ謨・, r.text)
        except Exception as e:
            QMessageBox.warning(self, "菫晏ｭ伜､ｱ謨・, str(e))
    ...
    def on_save_llm(self) -> None:
        try:
            ...
            if r.ok:
                QMessageBox.information(self, "菫晏ｭ・, "LLM險ｭ螳壹ｒ菫晏ｭ倥＠縺ｾ縺励◆")
            else:
                QMessageBox.warning(self, "菫晏ｭ伜､ｱ謨・, r.text)
        except Exception as e:
            QMessageBox.warning(self, "菫晏ｭ伜､ｱ謨・, str(e))
...
class OutputPage(QWidget):
        ...
        try:
            ...
            if r.ok:
                ...
                if data.get("default_output_dir"):
                    self.info.setText(f"譌｢螳・ 繧ｫ繧ｹ繧ｿ繝 ({cd})")
                else:
                    self.info.setText(f"譌｢螳・ 繝・ヵ繧ｩ繝ｫ繝・({default_dir})")
            else:
    ...
    def on_export(self) -> None:
        ...
        try:
            ...
            if r.ok:
                path = r.json().get("csv")
                QMessageBox.information(self, "蜃ｺ蜉帛ｮ御ｺ・, f"蜃ｺ蜉帙＠縺ｾ縺励◆:\n{path}")
                self.refresh_history()
            else:
    ...
    def choose_dir(self) -> None:
        d = QFileDialog.getExistingDirectory(self, "蜃ｺ蜉帛・繝輔か繝ｫ繝驕ｸ謚・, self.dest_edit.text() or str(Path.home()))
        if d:
...
class ScanPage(QWidget):
    ...
    def choose_folder(self) -> None:
        folder = QFileDialog.getExistingDirectory(self, "繝輔か繝ｫ繝驕ｸ謚・, self.watch_folder.text() or str(Path.home()))
        if folder:
    ...
    def start_scan(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "繧ｹ繧ｭ繝｣繝ｳ髢句ｧ句､ｱ謨・, str(e))

        if not ok:
            QMessageBox.warning(self, "繧ｹ繧ｭ繝｣繝ｳ髢句ｧ句､ｱ謨・, "ScanSnap Home繧定ｵｷ蜍輔〒縺阪∪縺帙ｓ縺ｧ縺励◆縲・)
...
class ReviewPage(QWidget):
    def __init__(self, company: str) -> None:
        ...
        form.addRow("譌･莉・YYYY/MM/DD)", self.date)
        form.addRow("驥鷹｡・, self.amount)
        form.addRow("鞫倩ｦ・, self.summary)
        form.addRow("蛟滓婿蜍伜ｮ・, self.debit)
        form.addRow("雋ｸ譁ｹ蜍伜ｮ・, self.credit)
        ...
        self.invoice = QComboBox(); self.invoice.addItems(["", "驕ｩ譬ｼ", "髱樣←譬ｼ", "髱櫁ｪｲ遞・])
        form.addRow("蛟滓婿陬懷勧遘醍岼", self.debit_sub)
        form.addRow("雋ｸ譁ｹ陬懷勧遘醍岼", self.credit_sub)
        form.addRow("隲区ｱよ嶌蛹ｺ蛻・繧､繝ｳ繝懊う繧ｹ)", self.invoice)
        ...
        btn_row = QHBoxLayout()
        later_btn = QPushButton("蠕後〒遒ｺ隱・)
        ok_btn = QPushButton("OK")
        del_btn = QPushButton("蜑企勁")
    ...
    def on_ok(self) -> None:
        ...
        if r.ok:
            QMessageBox.information(self, "菫晏ｭ・, "遒ｺ螳壹・蟄ｦ鄙偵＠縺ｾ縺励◆")
        else:
            QMessageBox.warning(self, "繧ｨ繝ｩ繝ｼ", r.text)
        self.refresh()
